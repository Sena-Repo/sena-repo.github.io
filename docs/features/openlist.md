...
Sena Repo 可以把 OpenList 作为游戏库或 Steam 补丁库的文件来源，适合资源已经放在网盘、NAS 聚合目录或 OpenList 管理目录中的场景。

## 两种部署方式

### 本地挂载 OpenList 目录

如果宿主机已经把 OpenList / 网盘目录挂载成本地路径，可以按普通本地目录挂入容器：
...
如果不想在服务端挂载游戏目录，可以只挂载 `/data`，之后在客户端「扫描设置」里添加 OpenList 服务器和目录：
